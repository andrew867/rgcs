...
core modules, plus the hub subcommands (`doctor`, `serve`, `modules`,
`coordinate`, `predictions`) from the integration lane:
...
rgcs-lab memory benchmark          # packaged corpus (works from a wheel)
...
rgcs-lab modules
rgcs-lab serve                     # loopback only; --allow-remote to override
```
