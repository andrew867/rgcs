...
touching the codec core. `python -m rgcs_archive adapter-info voyager2_pws` reports the
blocker rather than pretending the lane works.

## Blocker

The PDS4 bundle/label reader is not implemented. Waveform samples, ancillary
bytes, timing, gaps and frame identifiers must be kept together, and calibrated
spectral products must never be silently treated as raw waveform frames.
