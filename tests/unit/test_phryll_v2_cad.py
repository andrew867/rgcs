...
def test_scad_write_and_optional_openscad(tmp_path, design):
    _, cone, coil = design
    receipt = write_scad(render_scad(cone, coil), tmp_path / "d.scad")
    assert len(receipt["sha256"]) == 64
    stl = export_stl_if_openscad(tmp_path / "d.scad",
                                 tmp_path / "d.stl")
    assert stl.get("status") in ("rendered", "unavailable", "failed")
